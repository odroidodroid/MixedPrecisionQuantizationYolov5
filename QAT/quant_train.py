...
#from dataset.pycocotools import coco, cocoeval
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to('cuda')  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to('cuda')  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs = imgs.to('cuda', non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, 'cuda').half(),
...
def parse_opt(config=None, known=False):
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / '../dataset/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    ...
    opt.__dict__.update(loaded_yml)
    return opt
...
if __name__ == "__main__":
    opt = parse_opt("./config/config.yml")
    main(opt)
